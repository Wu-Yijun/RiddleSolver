...
/**
 * This is the Nizi Code to decode sentences.
...
import * as nizi_export from "./nizi-code.ts";
import { assertEquals } from "jsr:@std/assert";
Deno.test("Real Nizi Code", () => {
  const nizi = new NiziCode();
  const coder = new nizi_export.NiziCode(
    "./designer/assets/nizi_code.txt",
    "code",
  );
  ...
  for (const [name, code] of tree) {
    assertEquals(nizi.map.decode.get(code), name);
    assertEquals(nizi.map.encode.get(name), code);
    console.log("OK:", name, code);
  }
});

Deno.test("Web of Data", () => {
  const nizi = new nizi_export.NiziCode(nizi_export.nizi_code_path, "code");
  console.log(nizi.sentence("24038 -1807455 778 256065152 -7912 86 -237865."));
});

export { NiziCode, WebDecoder };
